...
import me.drex.magic_particles.util.LocalizedMessage;
...
public class MagicParticlesCommand {
    ...
    private static int sendList(CommandContext<CommandSourceStack> ctx) {
        ...
        MutableComponent list = ComponentUtils.formatList(
            particles.entrySet(),
            LocalizedMessage.localized("text.magic_particles.overview.list.separator", ctx.getSource()),
    ...
    private static int reload(CommandContext<CommandSourceStack> ctx) {
        if (ParticleManager.load()) {
            Map<String, MagicParticle> particles = ParticleManager.particles();
            ctx.getSource().sendSuccess(() -> LocalizedMessage.localized("text.magic_particles.reload", ctx.getSource()), false);
            return particles.size();
        } else {
            ctx.getSource().sendFailure(LocalizedMessage.localized("text.magic_particles.error", ctx.getSource()));
        }
        return 1;
    }
    ...
    private static int setParticle(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (!ParticleManager.particles().containsKey(particle)) {
            ctx.getSource().sendFailure(LocalizedMessage.localized("text.magic_particles.unknown", ctx.getSource()));
            return 0;
        }
        ...
        if (!Permissions.check(ctx.getSource(), "magic-particles.particle." + particle, true)) {
            ctx.getSource().sendFailure(LocalizedMessage.localized("text.magic_particles.missing_permission", Map.of(
    ...
    private static int disable(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        ctx.getSource().sendSuccess(() -> LocalizedMessage.localized("text.magic_particles.unset", ctx.getSource()), false);
        return 1;
    }
}
